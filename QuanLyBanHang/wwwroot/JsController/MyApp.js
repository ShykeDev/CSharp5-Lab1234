...
myApp.run(function ($rootScope, $http) {
    ...
    $rootScope.getCartCount = function () {
        $http.get("/Home/CartCount?id=" + getCookie("ID")).then(function (response) {
            $rootScope.cardcount = Number(response.data);
        });
    }
    $rootScope.getCartCount();

    $rootScope.Search = function (name) {
        if (name == "") return;
        if (name == undefined) return;
        location.href='/Home/Search?name=' + name;
    }
