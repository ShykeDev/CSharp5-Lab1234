myApp.controller('GioHangCtrl', function ($rootScope, $scope, $http) {
    ...
    $scope.MuaLai = function (id) {
        var spMuaLai = $scope.ListHoaDon.find(x => x.id == id);
        for (let index = 0; index < spMuaLai.hoaDonChiTiets.length; index++) {
            const element = spMuaLai.hoaDonChiTiets[index];
            console.log(element);
            $scope.GioHangChiTiet.thuocTinh = element.thuocTinh;
            $scope.GioHangChiTiet.userId = getCookie("ID");
            $scope.GioHangChiTiet.sanPhamId = element.idSanPham;
            $scope.GioHangChiTiet.soLuong = element.soLuong;
