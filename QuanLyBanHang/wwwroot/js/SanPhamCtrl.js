    ...
    $.ajax({
        ...
        success: function (response) {
            if (response == null || response == undefined || response.length == 0) {
                ...
                ListDanhMuc = response.danhMuc;
                ListDanhMucChiTiet = response.danhMucChiTiet;
                ListThuocTinh = response.thuocTinh;
                console.log(response)
                ListSanPham.forEach(addTable);
                ...
                function addTable(item) {
                    ...
                    if (item.soLuong != 0) {
                        trangThaiCell.textContent = item.trangThai == 2 ? "Đang hiển thị" : "Ẩn";
                    } else {
                        trangThaiCell.textContent = "Hết hàng";
                    }
                ...
                ListDanhMuc.forEach(addDanhMuc);
                function addDanhMuc(item) {
                    ...
                    label.focus = item.name;
                    label.htmlFor = item.id;
                    label.textContent = item.name;
                    document.getElementById("DanhMuc").appendChild(input);
                    document.getElementById("DanhMuc").appendChild(label);
                }
            }
        },
        error: function () {
...
function OnThemSanPham() {
    ...
    for (var i = 0; i < ListDanhMuc.length; i++) {
        if (document.getElementById(ListDanhMuc[i].id).checked) {
            CheckedList.push(ListDanhMuc[i].id);
        }
    }
    $.ajax({
...
function SuaSanPham(Item) {
    ...
    for (var j = 0; j < ListDanhMucChiTiet.length; j++) {
        if (ListDanhMucChiTiet[j].idSanPham == sanPhamModel.id) {
            document.getElementById(ListDanhMucChiTiet[j].idDanhMuc).checked = true;
        }
    }
    function addTC(item) {
