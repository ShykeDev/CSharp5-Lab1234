...
namespace QuanLyBanHang.Controllers
{
    public class HomeController : BaseController
    {
        ...
        public IActionResult FindSp(string danhMuc, string name)
        {
            MainDbContext _context = new MainDbContext();
            if (danhMuc != null)
            {
                var tmp = _context.DanhMucs.FirstOrDefault(dm => dm.Name == danhMuc);
                var SpList = _context.DanhMucChiTiets.Include(sp => sp.SanPham).Where(sp => sp.idDanhMuc == tmp.ID);
                var listSP = new List<SanPham>();
                foreach (var item in SpList) {
                    listSP.Add(item.SanPham);
                }
                var imgs = _context.ItemImages.ToList();
                
                return Json(listSP);
            }
            if (name != null) {
                var SpList = _context.SanPhams.Include(sp => sp.anhs).Where(sp => sp.Name.Contains(name)).ToList();
                return Json(SpList);
            }
            return View();
        }

        public IActionResult CartCount(Guid id)
        {
            try
            {
                MainDbContext _context = new MainDbContext();
                ...
                {
                    return Json(_context.GioHangChiTiets.Count(gh => gh.UserID == id));
        ...
        [HttpGet]
        public async Task<JsonResult> RemoveSanPhamGioHang(Guid id)
        {
            MainDbContext _context = new MainDbContext();
            try
            {
                if (HttpContext.Request.Cookies["ID"] == "" || HttpContext.Request.Cookies["ID"] == null)
                {
                    List<GioHangChiTiet?> gioHangList = JsonSerializer.Deserialize<List<GioHangChiTiet>>(HttpContext.Request.Cookies["GioHang"]);
                    HttpContext.Response.Cookies.Delete("GioHang");
                    gioHangList.Remove(gioHangList.First(gh => gh.ID == id));
                    HttpContext.Response.Cookies.Append("GioHang", JsonSerializer.Serialize(gioHangList));
                    return Json(true);
                }
                _context.GioHangChiTiets.Remove(_context.GioHangChiTiets.First(gh => gh.ID == id));
                await _context.SaveChangesAsync();
                return Json(true);
            }
            catch (Exception)
